...
async function showInput() {
	...
	// Функция 'nextTick' ожидает, когда произойдет ререндеринг компонента
	// Т.к. мы изменили span на input, нужно подождать, когда отрисуется input
	await nextTick();
...
function updateInput() {
	...
	if (props.column.title === state.columnTitle) return;
